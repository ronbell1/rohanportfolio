...
const ExpCard: React.FC<ExpCardProps> = ({
  ...
  <motion.div
    ...
    className="group relative p-6 sm:p-8 md:p-10 border-2 border-electric-blue rounded-[2.5rem] bg-white/60 transition-all hover:bg-white puzzle-shadow flex flex-col gap-4 sm:gap-6"
  >
    <div className="flex flex-col gap-2">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
        <span className="text-[9px] sm:text-[10px] font-black electric-blue uppercase tracking-[0.2em] bg-electric-blue/5 px-3 sm:px-4 py-1.5 rounded-full border border-electric-blue/10 whitespace-nowrap">
          {period}
        </span>
        <SketchedPuzzleIcon size={24} opacity={0.2} />
      </div>
      <h3 className="font-serif text-2xl sm:text-3xl md:text-4xl leading-tight group-hover:electric-blue transition-colors">
        {title}
      </h3>
    ...
    <div className="flex-1 space-y-3 sm:space-y-4">
      <ul className="space-y-2 sm:space-y-3">
        {points.map((point, idx) => (
          <li
            key={idx}
            className="flex gap-3 sm:gap-4 text-xs sm:text-sm leading-relaxed font-medium opacity-80"
          >
            <span className="electric-blue text-xs pt-1 flex-shrink-0">✦</span>
            <span>{point}</span>
          </li>
        ))}
      </ul>
    ...
    {skills && (
      <div className="pt-4 sm:pt-6 border-t border-electric-blue/10 flex flex-wrap gap-2">
        {skills.map((skill) => (
          <span
            key={skill}
            className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest bg-white border border-electric-blue/20 text-electric-blue px-2.5 sm:px-3 py-1 rounded-lg"
          >
            {skill}
          </span>
        ))}
...
export const Experience: React.FC = () => {
  return (
    <section id="experience" className="space-y-12 sm:space-y-16 py-8 sm:py-12">
      <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-4 sm:gap-6 border-b-2 border-electric-blue pb-6 sm:pb-8">
        <motion.h2
          initial={{ opacity: 0, x: -20 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl electric-blue"
        >
          career path.
        </motion.h2>
        <motion.p
          ...
          className="max-w-xs text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] opacity-40 italic text-right"
        >
