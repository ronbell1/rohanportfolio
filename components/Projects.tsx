...
const ProjectBlock: React.FC<ProjectBlockProps> = ({
  ...
  <motion.div
    ...
    className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 items-center py-12 sm:py-20 border-b border-black/5 last:border-none group perspective-1000"
  >
    <div className="space-y-6 sm:space-y-8 order-2 md:order-1">
      <div className="space-y-3 sm:space-y-4">
        <motion.h3
          ...
          className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl group-hover:electric-blue transition-colors leading-tight"
        >
          {title}
        </motion.h3>
        <motion.p
          ...
          className="text-lg sm:text-xl md:text-2xl font-serif italic opacity-60"
        >
          {desc}
        </motion.p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {tags.map((tag, i) => (
          <motion.span
            ...
            className="text-[9px] sm:text-[10px] font-black border-2 border-electric-blue/10 px-3 sm:px-4 py-1.5 rounded-full uppercase tracking-widest bg-white/50 hover:bg-electric-blue hover:text-white transition-colors"
          >
            {tag}
          </motion.span>
        ))}
      ...
      <ul className="space-y-2 sm:space-y-3 opacity-80">
        {details.map((detail, idx) => (
          <motion.li
            ...
            className="flex gap-3 sm:gap-4 text-xs sm:text-sm font-medium"
          >
            <span className="electric-blue flex-shrink-0">✦</span>{" "}
            <span>{detail}</span>
          </motion.li>
        ))}
      </ul>
      ...
      <div className="flex gap-3 sm:gap-4 pt-2 sm:pt-4">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="flex items-center gap-2 sm:gap-3 bg-electric-blue text-white px-6 sm:px-8 py-3 sm:py-4 rounded-2xl font-black uppercase text-[9px] sm:text-[10px] tracking-widest puzzle-shadow hover:brightness-110 whitespace-nowrap"
        >
  ...
  return (
    <section id="projects" className="space-y-12 sm:space-y-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6 border-b-2 border-electric-blue pb-6 sm:pb-8">
        <h2 className="font-serif text-5xl sm:text-6xl md:text-7xl lg:text-8xl electric-blue">
          projects.
        </h2>
        <p className="max-w-xs text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] opacity-40 italic text-right">
